...
\pysauce{AdvectionDGFluxTerm} corresponds with discretized term \eqref{eq:hyp_flux_app} where 
...
\end{itemize}
The general hyperbolic term is implemented in class \pysauce{NonlinearHyperbolicDGFluxTerm}
...
\ref{lst:laplace} where \pysauce{"dw_dg_diffusion_flux.i.Omega(D.val, p, v)"} corresponds to 
$a^R_\mathrm{diff}(\mathbf{p})$ and mode \pysauce{'avg_state'} (\ref{lsta:avg_state}), and 
\pysauce{"dw_dg_diffusion_flux.i.Omega(D.val, v, p)"} corresponds to $a^L_\mathrm{diff}(\mathbf{p})$ 
