...
\begin{landscape}
    \begin{table}[p!]
        ...
        \begin{tabular}{lccc}
            ...
            \midrule
            \pysauce{AdvectionDGFluxTerm} & \pysauce{"dw_dg_advect_laxfrie_flux"}
            ...
            & $\int_{\partial{T^k}} \vec{n} \cdot f^{*} (p_{in}, p_{out})\cdot\psi_j$ \\  
            \pysauce{NonlinearScalarDotGradTerm} & \pysauce{"dw_ns_dot_grad_s"} 
            ...
            \end{tabular}  \\ 
            \pysauce{DiffusionInteriorPenaltyTerm}& \pysauce{"dw_dg_interior_penalty"} 
            & $a^P_\mathrm{diff}(\mathbf{p})$ & $\elbint \sigma [P^k_i\psi_i]\psi_j$ \\ 
            \midrule
...
\begin{itemize}
    \item abstract base class \pysauce{DGMultiStageTS} and two solvers used in numerical  
    experiments: 
...
stepping solvers. For technical we also created \pysauce{DGVariable} class this was 
necessary to bypass classical FE treatment of boundary conditions, otherwise it is 
similar to original SfePy \pysauce{Variable} class and we its omit detailed description.
...
\begin{itemize}
    \item \pysauce{get_both_facet_state_vals} -- which returns values of state on 
    opposing sides of the boundary for each element
    \item \pysauce{get_both_facet_base_vals} -- which returns values of basis functions 
    on opposing sides of the boundary for each element
    \item \pysauce{get_facet_neighbor_idx} -- which returns indicies of cell neighbors 
...
\pysauce{get_n_el_nod}, which returns number of basis functions for given order , 
dimension and type of basis, and generator \pysauce{iter_by_order} 
...
\end{lstlisting}
\begin{itemize}
    \item[\ref{lsta:yield}] \pysauce{yield} keyword turns function into generator usable 
    in for cycles, for example in Listing \ref{lst:limiter_2D}.
    \item[\ref{lsta:extended}] \pysauce{extended} flag differentiates simplex basis vrom 
...
\end{lstlisting}
\begin{itemize}
    \item[\ref{lsta:advflux_central}] \pysauce{numpy.einsum} uses Einstein 
...
General hyperbolic term is implemented in class \pysauce{NonlinearHyperbolicDGFluxTerm}



\subsection{Diffusion flux term implementation}
\label{se:diff_flux_term_imp}
\pysauce{DiffusionDGFluxTerm} implements both terms in \eqref{eq:diff_left_approx} and 
...
\pysauce{"dw_dg_diffusion_flux.i.Omega(D.val, p, v)"} corresponds to 
$a^R_\mathrm{diff}(\mathbf{p})$ and mode \pysauce{'avg_state'} (\ref{lsta:avg_state}) and 
\pysauce{"dw_dg_diffusion_flux.i.Omega(D.val, v, p)"} corresponds to 
$a^L_\mathrm{diff}(\mathbf{p})$ 
and mode \pysauce{'avg_virtual'} (\ref{lsta:avg_virtual})
\setcounter{lstannotation}{0}
...
We list implementation of 2D limiter for reference in Listing \ref{lst:limiter_2D}. Limiter is implemented according to \Cref{se:limiters}.
...
As demonstrated in \Cref{se:time_theory} explicit DG FEM requires explicit 
time 
