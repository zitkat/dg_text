...
method and then derive the discretization of advection term $\vec{a}\cdot\nabla(p)$, 
general 
nonlinear hyperbolic term $\nabla\cdot \vec{f}(p)$, diffusion term $\nabla \cdot (D  
\nabla p)$ and source term $g(x)$ occurring in various PDEs namely, from the 
